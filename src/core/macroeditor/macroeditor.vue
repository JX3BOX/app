<template>
    <div id="app">
        <Header></Header>
        <Breadcrumb name="宏编辑器" slug="macroeditor" root="/app/macroeditor" :feedbackEnable="true" :crumbEnable="true">
        ...
        <Main class="m-macroeditor" :withoutRight="true" :withoutLeft="true">
            <h1 class="m-macroeditor-title">
                智能宏编辑器
                <a href="/tool/265" target="_blank" class="u-help el-button el-button--success is-plain el-button--mini"> <i class="el-icon-s-management"></i> 宏语法参考手册 </a>
            </h1>
            <div class="m-editor">
                <el-row>
                    <el-col :span="12">
                            ...
                            <el-form class="u-cmds" ref="form" :model="form" label-width="80px">
                                ...
                                <el-form-item label="条件释放">
                                    ...
                                    >
                                        ...
                                        <el-dialog title="实际逻辑" :visible.sync="logicDialogVisible" :width="Math.min(640, this.windowInnerWidth * 0.9) + 'px'">
                                            <span>{{ logicDialog }}</span>
                                            <span slot="footer" class="dialog-footer">
                                                <el-button type="primary" @click="logicDialogVisible = false">确 定</el-button>
                                            </span>
                                        </el-dialog>
                                    </template>
                                </el-form-item>
                                <template v-if="isConditional">
                                    <el-form-item v-for="(condition, index) of conditions" :key="index" :label="`条件${index + 1}`">
                                        ...
                                        <el-row :gutter="6">
                                            <el-col :span="16">
                                                <el-select v-model="condition.name" placeholder="判断项目" @change="onConditionChange(index)">
                                                ...
                                                <el-row :gutter="2">
                                                    <el-col :span="10" v-if="needsConditionParams.subname.includes(condition.name)">
                                                        <el-input v-model="condition.subname" placeholder="名称"></el-input>
                                                    </el-col>
                                                    <el-col :span="7" v-if="needsConditionParams.relation.includes(condition.name)">
                                                        <el-select v-model="condition.relation" v-if="!needsConditionParams.relationRestricted.includes(condition.name)">
                                                    ...
                                                    <el-col :span="7" v-if="needsConditionParams.value.includes(condition.name)">
                                                        <el-input v-model="condition.value" placeholder="值"></el-input>
                                                    </el-col>
                                                </el-row>
                                            </el-col>
                                            <el-col :span="7">
                                                <el-button type="danger" plain circle icon="el-icon-minus" @click="clickMinusCondition(index)" v-if="conditions.length > 1"></el-button>
                                                <el-button
                                                    type="primary"
                                                    plain
                                                    circle
    ...
    data: function() {
        return {
            ...
            needsConditionParams: {
                subname: ["buff", "buff_level", "nobuff", "bufftime", "skill_energy", "skill", "noskill", "skill_notin_cd", "tbuff", "tbuff_level", "tnobuff", "tbufftime"],
                relation: [
                    "buff_level",
                    "tbuff_level",
                    "bufftime",
