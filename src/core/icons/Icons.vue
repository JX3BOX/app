                ...
                <el-tabs v-model="activeTabName" type="card" @tab-click="handleClick">
                        ...
                        <ul class="m-icon-list">
                            ...
                            <li v-for="(icon, index) in iconsList" :key="index" @click="handleAddFavorite(index, icon)" @mouseleave="handleMouseLeaveIcon">
                                <i class="u-pic">
                                    ...
                                    <!-- <img class="u-img" src="https://console.cnyixun.com/icon/2589.png" alt="JX3BOX剑三图标库" /> -->
                    ...
                    <el-tab-pane label="收藏图标" name="favicon" lazy :loading="isSynchronizing">
                        <ul class="m-icon-list" v-if="activeTabName === 'favicon'">
                            <el-alert title="本地有收藏图标未同步到服务器" type="info" center show-icon v-if="faviconNeedsSync"><el-button type="text" @click="syncFavicon">点此同步未登录收藏数据</el-button></el-alert>
                            <transition name="el-fade-in" v-for="(icon, index) in faviconsList" :key="index">
                            <li>
                                <i class="u-pic">
                                    <el-image :src="`${JX3BOXIconPath}${icon}.png`" class="u-img" lazy>
                                        <div slot="placeholder" class="image-slot"><i class="el-icon-loading"></i></div>
                    ...
                    <el-tab-pane label="表情包" name="emoji" lazy>表情包</el-tab-pane>
                </el-tabs>
            </div>
...
export default {
    ...
    data: function() {
        return {
            ...
            iconsList: [],
            clickedIndex: -1,
            JX3BOXIconPath: JX3BOX.__iconPath,
            faviconsList: null,
            localFaviconsList: [],
    ...
    methods: {
        async prepareMounted() {
            ...
            let allIconsListUrl = JX3BOX.__staticPath.jsdelivr+'jx3-icon/icon.json';
            let getAllIconsList = axios(allIconsListUrl, 'GET');
            
            let allEmojiListUrl = JX3BOX.__ossMirror + 'data/emotion.json'
