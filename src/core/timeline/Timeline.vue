		...
		<main class="c-main p-app-timeline">
			<div class="c-table">
				<div v-if="type === 'lanren'">
					<el-table :data="list" style="width: 100%">
						<el-table-column prop="caster.szName" label="释放者"> </el-table-column>
						<el-table-column prop="nTime" label="时间">
							<template slot-scope="scope">
								<div v-if="ms"> {{ scope.row.nTime }}</div>
								<div v-else> {{ scope.row.nTime | time_Milli }}</div>
							</template>
						</el-table-column>
						<el-table-column prop="nType" label="类型"> </el-table-column>
						<el-table-column prop="nLevel" label="技能等级"> </el-table-column>
						<el-table-column prop="dwSkillID" label="技能ID"> </el-table-column>
					</el-table>
				</div>
				<div v-else-if="type == 'old'">
					<el-table :data="list" style="width: 100%">
						...
						<el-table-column prop="ev_3" label="类型" width="180"> </el-table-column>
          ...
          暂无
			...
			<aside class="c-sidebar-right c-sidebar">
				...
				<div v-if="type == 'lanren'">
					<el-button plain @click="toMs" v-if="ms">切换成秒</el-button>
					<el-button plain @click="toMs" v-else>切换成毫秒</el-button>
				</div>
			</aside>
		</main>
	</div>
	...
	methods: {
		...
		read(f) {
			...
			rd.onload = (e) => {
				...
				console.log('list___________________文件上传转换后的数据', this.list)
